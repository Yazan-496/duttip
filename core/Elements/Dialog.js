...
export default function DraggableDialog({ ...res }) {
  ...
  return (
    <div>
      <Dialog className="w-full" open={res.data.show} onClose={handleClose}>
        ...
        <DialogContent className="flex">
          {res.data?.data?.length > 0 ? (
            <Alert className="w-full flex max-w-full" severity={res.type}>
              {t("user.you_won't_to_delete_this")}
              {res.text}
              {"/"}
              {res.texts}{" "}
              {res?.data?.data?.map((one) => {
                return (
                  <div className="font-[700] text-red-700">{one?.name}</div>
                );
              })}
            </Alert>
          ) : (
            <Alert className="w-full max-w-full flex" severity={res.type}>
              {t("user.you_won't_to_delete_this")} {res.text}
              <div className="font-[700] text-red-700">
                {res.data?.data?.name}
              </div>
            </Alert>
          )}
        ...
        <DialogActions className="flex items-center justify-center font-[700] text-gray-500">
          <ButtonClose
            autoFocus
            onClick={handleClose}
            text={res.cancelText || t("user.cancel")}
          />
          <ButtonSave
            onClick={handleConfirm}
            text={res.confirmText || t("user.confirm")}
          />
        </DialogActions>
      </Dialog>
    </div>
