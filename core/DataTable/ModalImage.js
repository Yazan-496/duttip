...
import ButtonClose from "core/Elements/ButtonClose";
...
const ModalImage = ({
  ...
  const handleSetRegion = () => {
    ...
    if (selectedAnswer) {
      const findIndex = selectedPoints?.findIndex(
        (one) => one?.answerId === selectedAnswer?.value,
      );
      if (findIndex === -1) {
  ...
  const _getAnswerForThisQuestion = (questionId) => {
    ...
    if (
      answers?.filter((answer) => answer?.questionId === questionId)?.length > 0
    ) {
      // console.log("in")
  ...
  return (
    <TEModal
      className="absolute left-0 top-0 backdrop-blur-[5.5px] overflow-hidden bg-transparent"
      show={openModal}
      setShow={_openModal}
    >
      <TEModalDialog
      ...
      >
        <TEModalContent className="h-full w-full ">
          <div
            className={`${
              close ? "w-[200px]" : "w-[10px]"
            } border border-l-gray-200 border-[3px] fixed shadow-lg z-10 bg-white h-full top-0 right-0 flex flex-col`}
          >
            ...
            >
              <ArrowSvg width={"3vh"} height={"5vh"} rotate={close ? 0 : 180} />
            </div>
